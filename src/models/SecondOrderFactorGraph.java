...
public class SecondOrderFactorGraph {
	...
	public SecondOrderFactorGraph(AbstractCorpus corpus, SecondOrderPotentialFunction potentialFunction, 
				...
				for(int sp : fiter.states(sequenceID, i-1))
					for(int spp : fiter.states(sequenceID, i-2))
						edgeScore[i][s][sp][spp] = backoff + 
								potentialFunction.computeEdgeScore(sequenceID, i, s, sp, spp, parameters);
			}
		}
	}
	...
	public void computeMarginals() {
		...
		for(int i = 1; i <= length; i++)
			for(int s : fiter.states(sequenceID, i))
				for(int sp : fiter.states(sequenceID, i-1)) {
					len = 0;
					for(int spp : fiter.states(sequenceID, i-2)) {
						dpTemplate[len++] = alpha[i-1][sp][spp] + edgeScore[i][s][sp][spp] + nodeScore[i][s];
					}
		...
		for(int i = length; i > 0; i--) 
			for(int sp : fiter.states(sequenceID, i - 1)) 
				for(int spp : fiter.states(sequenceID, i - 2)) {
					len = 0;
					for(int s : fiter.states(sequenceID, i)) {
						dpTemplate[len++] = beta[i][s][sp] + edgeScore[i][s][sp][spp] + nodeScore[i][s];
					}
		...
		for(int i = 1; i <= length; i++) {
			for(int s : fiter.states(sequenceID, i)) {
				...
				for(int sp : fiter.states(sequenceID, i - 1)) { 
					for(int spp : fiter.states(sequenceID, i - 2)) {
						edgeMarginal[i][s][sp][spp] = alpha[i-1][sp][spp] + beta[i][s][sp] + 
										 	edgeScore[i][s][sp][spp] + nodeScore[i][s] - logNorm;
					}
					dpTemplate[len++] = alpha[i][s][sp] + beta[i][s][sp] - logNorm;
				}
	...
	public void addToEmpirical(int sequenceID, int[] gold, double[] empirical) 
	{
		for(int i = 0; i <= gold.length; i++) {
			...
			potentialFunction.addToEmpirical(sequenceID, i, s, sp, spp, empirical, 1.0);
		}
	}
				...
				for(int sp : fiter.states(sequenceID, i-1))
					for(int spp : fiter.states(sequenceID, i-2)) {
						if(!Double.isInfinite(edgeMarginal[i][s][sp][spp])) {
							double marginal = Math.exp(edgeMarginal[i][s][sp][spp]) * multiplier;
							potentialFunction.addToEmpirical(sequenceID, i, s, sp, spp, empirical, marginal);
						}
					}
	}
