...
public class SparseSimilarityGraph {
	...
	public SparseSimilarityGraph(String graphPath, int numNodes) throws NumberFormatException, IOException
	{
	...
	public SparseSimilarityGraph(String graphPath, int numNodes, boolean normalizeLaplacian) throws NumberFormatException, IOException
	{
		this(graphPath, numNodes);
		
		if(normalizeLaplacian) {
			System.out.println("Normalizing graph laplacian ...");

			for(int i = 0; i < numNodes; i++) {
				for(int j = 0; j < weights[i].size(); j++) {
					int e = edges[i].get(j);
					double w = weights[i].get(j) / Math.sqrt(degrees[e]) / Math.sqrt(degrees[i]); 
					weights[i].set(j, w);
				}
			}
		}
	...
	public double computeGoldViolation(AbstractCorpus corpus)
	{
		...
		for(int i = 0; i < corpus.numInstances; i++) {
			...
			for(int j = 0; j < seq.length; j++)
				if(seq.nodes[j] >= 0)
					goldDist[seq.nodes[j]][seq.tags[j]] += 1.0 / corpus.nodeFrequency[seq.nodes[j]];
		}
